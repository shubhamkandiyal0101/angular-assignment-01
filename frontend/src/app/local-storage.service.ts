...
export class LocalStorageService {
  ...
  // store item
  storeItem(itemKey,itemData) {
      console.log(' in service >> ',itemData)
      localStorage.setItem(itemKey,JSON.stringify(itemData))
  }
  // ends here ~ store item

  // get item
  getItem(itemKey) {
      // get item data
      const itemData = localStorage.getItem(itemKey)
      // ends here ~ get item data

      // return data
      if(itemData != '' && itemData != undefined && itemData != null) {
        const itemDataJson = JSON.parse(itemData);
        return {'message-type':'success','data':itemDataJson}
      } else {
        return {'message-type':'error','message':'Data not Found'}
      }
